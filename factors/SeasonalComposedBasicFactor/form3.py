...
from factors.Category import ComposedBasicFactorForm3
from factors.Frequency import SeasonalFrequency
from factors.sql import pl_sql_oracle

"""
季频组合基本面因子， Form3, X_change_pct - AT_change_pct形式
...
class SeasonalComposedBasicFactorF3(SeasonalFrequency, ComposedBasicFactorForm3):

    def __init__(self, factor_code= '', name= '', describe= ''):
        super().__init__(factor_code, name, describe)
        self.type = '季频组合基本面因子， Form3, X_change_pct - AT_change_pct形式'
    ...
    # 形式3, X_change_pct - AT_change_pct
    def init_factors(self):
        ...
        for n in self.numerator.itertuples(index=True, name='Pandas'):  # 分子
            for d in self.denominator.itertuples(index=True, name='Pandas2'):  # 分母
                factor_name = str(getattr(n, '英文简写')) + 'pct-' + str(getattr(d, '英文简写') + 'pct')
                factor_des = str(getattr(n, '描述')) + '较上期变化的百分数 - ' + str(getattr(d, '描述') +'较上期变化的百分数')
                factor_entity = SeasonalComposedBasicFactorF3(factor_code='CB%04d' % factor_code,
                                                                  name=factor_name,
                                                                 describe=factor_des)
    ...
    def find_components(self, file_path,secucode, date):
        ...
        try: # t7 LC_BalanceSheet
            ...
            components['LC_BalanceSheet_monthly_chg'] = self.calculate_change(components['LC_BalanceSheet_monthly'])
        ...
        try:# t8 LC_Staff
            ...
            components['LC_Staff_monthly_chg'] = self.calculate_change(components['LC_Staff_monthly'])
