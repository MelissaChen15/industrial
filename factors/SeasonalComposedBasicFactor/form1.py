...
class SeasonalComposedBasicFactorF1(SeasonalFrequency, ComposedBasicFactorForm1):
    ...
    # 形式1： X/AT
    def init_factors_2(self, numerator, denominator):
        factor_code = 0
        factor_entities = {}
        ...
        for n in numerator.itertuples(index=True, name='Pandas'):  # 分子
            for d in denominator.itertuples(index=True, name='Pandas2'):  # 分母
                ...
                try:
                    ...
                    for i in range(factor_values[factor_name].shape[0]):
                        if factor_values[factor_name][i] >= 10^6:
                            factor_values[factor_name] = np.log1p(factor_values[factor_name])
                            break
