...
class VolatilityFunc(object):
    ...
    def turnover_volume_std(self):
        res = pd.DataFrame(index=[])
        for j in self.window:
            res = pd.concat([res,self.TurnoverVolume_std.rolling(self.periodcoef*j).apply(np.std)],axis=1)
        return res

    def high_low_std(self):
        high_low_std_all = dict()
        for j in self.window:
            ...
            high_low_std_all['时间窗口'+str[j]] = res1
        return high_low_std_all
