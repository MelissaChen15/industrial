...
class TimeseriesFactorCal(object):
    ...
    def get_data_cal(self):
        ...
        s1 = sql.InputDataPreprocess(self.code_sql_file_path, ['TradingDay'],) # TradingDay作为表名会进行变量赋值，必须是合格的变量字符串
        ...
        for row in s1['TradingDay'].itertuples(index=True, name='Pandas'):
            try:
                ...
                print(getattr(row, 'TRADINGDAY'), ' done')

            except Exception as e:

                print(getattr(row, 'TRADINGDAY'), e)
        SML_and_HML.index = list(s1['TRADINGDAY']['TRADINGDAY'])  # s1是一个字典
        return SML_and_HML
