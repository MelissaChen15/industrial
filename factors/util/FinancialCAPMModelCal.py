...
import pandas as pd
import pyfinance
# 注意SMB,HML的时间区间与设定的因子值的时间段不一样，保守起见，计算得到的SMB等数据时间从2002年初开始，需进行日期匹配。
...
class CAPM_ModelCal(object):
    ...
    def sideBeta(self):
        common_index = list(set(self.ChangePCT.index).intersection(set(self.StockIndex.index)))
        common_index.sort()
        ...
        for j in self.rolling_window:
            ...
            for i in range(len(mean_data)):
                if mean_data[i] == mean_data[i]:  # 非nan情况
                    ...
                    upsidebeta = upsidebeta.append([upsidebeta_temp])
                    ...
                    downsidebeta = downsidebeta.append([downsidebeta_temp])

                    sidediffbeta = sidediffbeta.append([upsidebeta_temp-downsidebeta_temp])
                elif mean_data[i] != mean_data[i]:
                    upsidebeta = upsidebeta.append(pd.DataFrame([np.nan]))
                    downsidebeta = downsidebeta.append(pd.DataFrame([np.nan]))
                    sidediffbeta = sidediffbeta.append(pd.DataFrame([np.nan]))
        ...
        return upsidebeta_all,downsidebeta_all,sidediffbeta_all
    ...
    def CAPM_cal(self):
        ...
        for j in self.rolling_window:
            ...
            alpha1 = model.alpha
            beta1 = model.beta
            alpha1_all['滚动窗口(月)'+str(j)] = alpha1  # 索引是交易日期
            beta_all['滚动窗口(月)'+str(j)] = beta1  # 索引是交易日期
    ...
    def residuals_model_cal(self,alpha1_all,beta_all):
        ...
        for i in periods:
            ...
            for j in range(len(alpha.index)):
                data_y = self.ChangePCT[j:j+window_length]  # 取前不取后的
                data_x = self.StockIndex[j:j+window_length]
                alpha_j = alpha[alpha.index == alpha.index[j]].values  # shape(1,1)的array
                beta_j = beta[beta.index == alpha.index[j]].values  # 转换成array进行矩阵运算
                residuals = data_y.values - (np.dot(beta_j,data_x.values.reshape(1,len(data_x)))+alpha_j[0])   # 计算残差序列,type为array,shape必须是(1,数据长度)
                ...
                skew_resids.append(pd.DataFrame(residuals).transpose().skew()[0])  # 计算偏度 residuals 是shape(1,length)的array,需transpose
...
#     aaa = CAPM_ModelCal(ss.HS300ChangePCT,components['QT_Performance']['CHANGEPCT'],flag=1,rolling_window=[[3,6],[6,12]])
#     alpha1_all, beta_all = aaa.CAPM_cal()
#     res = aaa.residuals_model_cal(alpha1_all,beta_all)
#     upsidebeta, downsidebeta, sidediffbeta = aaa.sideBeta()
#
# ChangePCT= components['QT_Performance']['CHANGEPCT']
# StockIndex = ss.HS300ChangePCT
# rolling_window = [3,6]
