...
def dailyToweek(dailydata,code_sql_file_path):
    '''
    :param dailydata: 指数日频数据
    :param code_sql_file_path: 周频交易日序列的sql语句路径
    :return:
    '''
    s1 = getWeekTradingDay(code_sql_file_path)  # s1 is a dict which keys equal to tablename in pl_sql_oracle
...
class StockIndexGroup(object):
    ...
    def __init__(self,flag,code_sql_file_path):
        '''
        ...
        if flag ==1:
            ...
            self.HS300ChangePCT = dailyToweek(self.indexdata['StockIndexIF'][['TRADINGDAY','CLOSEPRICE']])  # 输出为周频
            self.SZ50ChangePCT = dailyToweek(self.indexdata['StockIndexIH'][['TRADINGDAY','CLOSEPRICE']])
            self.ZZ500ChangePCT = dailyToweek(self.indexdata['StockIndexIC'][['TRADINGDAY','CLOSEPRICE']])
