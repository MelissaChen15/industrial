...
class FF3_ModelCal(object):
    ...
    def FF3_cal(self):
        ...
        beta_all = dict()
        import pyfinance
        index_list = [self.StockIndex.index,self.HML.index,self.SMB.index]
        common_index = list(set(index_list[0]).intersection(*index_list[1:]))  # 时间序列对齐
        common_index.sort()
        data_x = pd.concat([self.StockIndex[common_index],self.HML[common_index],self.SMB[common_index]],axis=1)  # 拼接自变量
        for j in self.rolling_window:
            model = pyfinance.ols.PandasRollingOLS(self.ChangePCT, data_x, window=self.periodcoef * j)  # 注意y必须在前面
            alpha1 = model.alpha
            beta1 = model.beta
