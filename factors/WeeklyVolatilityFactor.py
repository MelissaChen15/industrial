...
class WeeklyVolatilityFactor(WeeklyFrequency,VolatilityFactor):
    ...
    def init_factors(self):
        ...
        count = 0000
        TO_cal =VolatilityFunc(pd.DataFrame(), pd.DataFrame(),pd.DataFrame(), pd.DataFrame(), 4,[3, 6, 9], pd.DataFrame(), pd.DataFrame())
        for i in self.target_methods:
            ...
            for n in name:
                ...
                entity = WeeklyVolatilityFactor(factor_code='WV%04d' % count,
                                        name=n,
                                        describe='')
