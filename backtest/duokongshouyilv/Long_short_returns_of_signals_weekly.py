...
def create_LSreturns_weekly(factor_data,auxiliary_data,factor_name):
    '''
    ...
    for i in np.arange(0,(len(time_series)-1)):
        ...
        # format_date = str(pd.to_datetime(str(time_series[i+1])).year)+str(pd.to_datetime(str(time_series[i])).month)
        ...
        returns_low = auxiliarytemp_data[auxiliarytemp_data['SECUCODE'].isin(low_stock)]['CHANGEPCTRW']
        returns_high = auxiliarytemp_data[auxiliarytemp_data['SECUCODE'].isin(high_stock)]['CHANGEPCTRW']
