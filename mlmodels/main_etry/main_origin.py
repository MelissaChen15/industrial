...
para = Para()
from mlmodels.regressors import Ridge_init
from mlmodels.classifiers import Logistic_init
...
if __name__ == '__main__':
    ...
    # 2. 初始化模型
    model, model_name= Logistic_init.init()

    # 3. 训练模型,保存模型
    model = train.train_class(model, model_name, X_train, X_cv, y_train, y_cv)

    # 4. 模型预测,保存预测结果
    n_days_in_test = predict.predict_class(model,model_name)
