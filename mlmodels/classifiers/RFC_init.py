...
def init():
    # Random Forest Classifier 参数
    #['bootstrap', 'class_weight', 'criterion', 'max_depth', 'max_features',
    # 'max_leaf_nodes', 'min_impurity_decrease', 'min_impurity_split', 'min_samples_leaf',
    ...
    # 初始化模型
    model = GridSearchCV(estimator= ensemble.RandomForestClassifier(random_state=para.seed), param_grid=param_grid, cv=cv_split, n_jobs=3)

    # 建立新的文件夹用于存储模型和预测结果
    if os.path.exists(para.path_results + "RFC") == False:
        os.mkdir(para.path_results + "RFC")
