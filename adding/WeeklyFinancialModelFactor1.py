...
from Category import FinancialModelFactor
from sql import pl_sql_oracle
from util.FinancialModelFuncProcess import FinancialModelFuncProcess
...
class WeeklyFinancialModelFactor(WeeklyFrequency,FinancialModelFactor):
    ...
    def init_factors(self):
        ...
        for i in range(len(self.target_methods)):
            factor_entities[self.target_methods[i]] = WeeklyFinancialModelFactor(factor_code=self.nameGroup[i],name=self.target_methods[i],describe='')
    ...
    def get_factor_values(self, components,code_sql_file_path_index,SMB_HML_file_path_daily,SMB_HML_file_path_weekly):
        """
        计算本类所有的因子
        ...
        TO_cal = FinancialModel_statsFunc(components['QT_Performance']['TRADINGDAY'],components['QT_Performance']['CHANGEPCTRW'],
                                          flag=2,code_sql_file_path1=code_sql_file_path_index)
        ...
        for i in marketindex:
            alpha1_all, beta_all, residuals_stats = TO_cal.FF3_model_stats(i,SMB_HML_file_path_daily,SMB_HML_file_path_weekly)
    ...
    def write_values_to_DB(self,  code_sql_file_path, data_sql_file_path,weeklyday_file_path,
                           code_sql_file_path_index, SMB_HML_file_path_daily, SMB_HML_file_path_weekly):
        """
        计算因子值并写入数据库中相应的数据表
        ...
        for row in s['secucodes'].itertuples(index=True, name='Pandas'):
            try:
                ...
                factor_values = self.get_factor_values(data,code_sql_file_path_index,SMB_HML_file_path_daily,SMB_HML_file_path_weekly)
...
if __name__ == '__main__':
    TO = WeeklyFinancialModelFactor(WeeklyFrequency, FinancialModelFactor)
    ...
    data_sql_file_path = r'D:\项目\FOF相关资料\wind研报\工具性代码\Chen\industrial\factors_ver4\StockFactorsCreating\factors\sql\sql_weekly_financialmodel_factor.sql' # 读取数据库数据的sql代码文件路径
    ...
    TO.write_values_to_DB(data_sql_file_path=data_sql_file_path, code_sql_file_path = code_sql_file_path, weeklyday_file_path=weeklyday_file_path,
                          code_sql_file_path_index = code_sql_file_path_index,SMB_HML_file_path_daily=SMB_HML_file_path_daily,
                          SMB_HML_file_path_weekly =SMB_HML_file_path_weekly)

    # components = TO.find_components(file_path= data_sql_file_path,
    #                                        table_name=['QT_Performance'],
    #                                        secucode=  'and t2.Secucode = \'' + '000001' + '\'',file_path2=weeklyday_file_path)