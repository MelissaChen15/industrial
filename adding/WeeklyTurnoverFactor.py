...
class WeeklyTurnoverFactor(WeeklyFrequency,TurnoverFactor):
    ...
    def init_factors(self):
        ...
        for i in range(len(self.target_methods)):
            factor_entities[self.target_methods[i]] = WeeklyTurnoverFactor(factor_code=self.nameGroup[i],name=self.target_methods[i],describe='')

        return factor_entities  # 不止一个因子

    def find_components(self, file_path1,file_path2, table_name,secucode = ''):
        """
        在数据库中查询计算本类因子需要的数据
        ...
        components = sql.InputDataPreprocess(file_path1, table_name, secucode )
    ...
    def get_factor_values(self, components):
        ...
        TO_cal = TurnoverFunc(components['QT_Performance']['TURNOVERRATERW'],components['QT_Performance']['CHANGEPCTRW'],
                              periodcoef=4,window=[3,6,9])
    ...
    def write_values_to_DB(self, code_sql_file_path, data_sql_file_path,weeklyday_file_path):
        ...
        for row in s['secucodes'].itertuples(index=True, name='Pandas'):
            try:
                data = self.find_components(file_path1= data_sql_file_path,file_path2= weeklyday_file_path,
...
if __name__ == '__main__':
    TO = WeeklyTurnoverFactor(WeeklyFrequency, TurnoverFactor)
    data_sql_file_path = r'D:\项目\FOF相关资料\wind研报\工具性代码\Chen\industrial\factors_ver4\StockFactorsCreating\factors\sql\sql_weekly_turnover_factor.sql' # 读取数据库数据的sql代码文件路径
