...
class WeeklyIdiosyncrasticFactor(WeeklyFrequency,IdiosyncrasticFactor):
    ...
    def init_factors(self):
        ...
        for i in range(len(self.target_methods)):
            factor_entities[self.target_methods[i]] = WeeklyIdiosyncrasticFactor(factor_code=self.nameGroup[i],name=self.target_methods[i],describe='')
...
if __name__ == '__main__':
    TO = WeeklyIdiosyncrasticFactor(WeeklyFrequency, IdiosyncrasticFactor)
    data_sql_file_path = r'D:\项目\FOF相关资料\wind研报\工具性代码\Chen\industrial\factors_ver4\StockFactorsCreating\factors\sql\sql_weekly_Idiosyncrastic_factor.sql' # 读取数据库数据的sql代码文件路径
