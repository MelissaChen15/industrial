...
from Category import IdiosyncrasticFactor
from sql import pl_sql_oracle
from util.IdiosyncrasticFuncProcess import IdiosyncrasticFuncProcess
...
class DailyIdiosyncrasticFactor(DailyFrequency,IdiosyncrasticFactor):
    ...
    def init_factors(self):
        ...
        for i in range(len(self.target_methods)):
            factor_entities[self.target_methods[i]] = DailyIdiosyncrasticFactor(factor_code=self.nameGroup[i],name=self.target_methods[i],describe='')
...
if __name__ == '__main__':
    TO = DailyIdiosyncrasticFactor(DailyFrequency, IdiosyncrasticFactor)
    data_sql_file_path = r'D:\项目\FOF相关资料\wind研报\工具性代码\Chen\industrial\factors_ver4\StockFactorsCreating\factors\sql\sql_daily_Idiosyncrastic_factor.sql' # 读取数据库数据的sql代码文件路径
